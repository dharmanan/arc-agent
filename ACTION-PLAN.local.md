...
- [x] Stable automation v1'i `USDC/EURC` swap + add liquidity + remove liquidity + rebalance lane'i olarak aç.
  ...
  - Sonuç: `backend/src/services/stableAutomationPolicy.js` şimdi target LP band mantığı da taşıyor. Varsayılan `20-30 USD` bandı için mevcut LP underweight kaldığında `top_up`, overweight ama sağlıklı kaldığında `trim_to_target`, hard safety guard bozulduğunda `full_exit` üretiyor; remove notional raporu da full position yerine `suggestedLpExitValueUsd` üstünden geliyor.
  ...
  - Doğrulama: `node --check backend/src/services/stableAutomationPolicy.js`, sentetik `top_up / trim_to_target / full_exit` policy probe'u, `node --check backend/src/queue/agentQueue.js` ve backend/frontend editor error check temiz geçti.
  - Deploy: Railway backend deploy alındı; frontend `vercel link --project arc-agent-frontend --yes` ile tekrar doğrulanıp production Vercel deploy yenilendi. Ardından `npm run smoke:prod` tekrar `0` ile geçti.
- [ ] `cirBTC` LP automation v1 aç: yalnız doğrulanmış pair ve position state üzerinden bootstrap, küçültme ve tam exit kuralları ekle.
...
- [ ] Lending paid görevlerini aç: supply, withdraw, borrow, repay, deleverage, collateral top-up, safe exit.
  - Yeni session başlangıç noktası (2026-05-21): buradan devam et. Bu turda manual lending backend rayında `deleverage` + `liquidate` wiring'i tamamlandı ve production backend deploy alındı. Sıradaki somut iş, bu action'ları public paid lane'e taşımak ve gerekiyorsa aynı turda `frontend/src/components/DeFiTab.jsx` içinde `recovery / liquidation` görünürlüğünü artırmak.
